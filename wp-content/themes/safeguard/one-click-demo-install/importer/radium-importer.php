	...
	class Radium_Theme_Importer {
			...
			<div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;">
			 ...
			 if( !empty($this->imported_demos) ) { ?>
			  	...
			  	<div style="background-color: #FAFFFB; margin:10px 0;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:90%; line-height:18px;">
			  		<p><?php _e('Demo already imported', 'PixHealth'); ?></p>
			  	</div><?php
			   	//return;
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = __('Import Again', 'PixHealth');

			} else {

				$button_text = __('Import Demo Data', 'PixHealth');
	    ...
	    public function set_demo_data( $file ) {
	        ...
	        if ( !class_exists( 'WP_Import' ) ) {
	            ...
	            $class_wp_import = get_template_directory().'/one-click-demo-install/importer/wordpress-importer.php';
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				__( 'Theme options Import file could not be found. Please try again.', 'PixHealth' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			__( 'Widget Import file could not be found. Please try again.', 'PixHealth' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'PixHealth' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = __( 'Site does not support widget', 'PixHealth' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = __( 'Widget already exists', 'PixHealth' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = __( 'Imported', 'PixHealth' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = __( 'Imported to Inactive', 'PixHealth' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'PixHealth' ); // show "No Title" if widget instance is untitled
