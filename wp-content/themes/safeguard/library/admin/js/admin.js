...
jQuery(document).ready(function () {
	...
	jQuery("#safeguard_pix_body_font, #safeguard_pix_headings_font, #safeguard_pix_menu_font").after('<div class="font-preview"></div>');
	...
	jQuery(function($) {
		$('#sortable-table-services tbody').sortable({
			axis: 'y',
			handle: '.column-order img',
			placeholder: 'ui-state-highlight',
			forcePlaceholderSize: true,
			update: function(event, ui) {
				...
				var data = {
					action: 'pixtheme_services_update_post_order',
					postType: $(this).attr('data-post-type'),
					order: theOrder
				};
...
// image Uploader Functions ##############################################
	...
	jQuery(".safeguard_pix_opts").parent().fadeIn(700);
