...
;(function($){
	...
	window.DH = {
		...
		ajaxSearchInit: function(){
			...
			$(document).on('click','.navbar-search-button.search-icon-mobile',function(e){
				e.stopPropagation();
				e.preventDefault();
		...
		woocommerceInit: function(){
			if($('.shop-loop-thumbnail-slider').length){
				DH.shop_thumbnail_slider_interval = null;
				DH.shop_thumbnail_slider_interval && clearInterval(DH.shop_thumbnail_slider_interval);
				$( ".shop-loop-thumbnail-slider .carousel").carousel({interval: 1500}).carousel('pause');
				$(document).on("mouseenter", ".woocommerce ul.products li.product figure", function() {
					var _this = $(this).find('.shop-loop-thumbnail-slider .carousel');;
					_this.carousel('next').carousel('cycle');
				});
				$(document).on("mouseleave", ".woocommerce ul.products li.product figure", function() {
					var _this = $(this).find('.shop-loop-thumbnail-slider .carousel');
					_this.carousel('pause');
				});
				$('.shop-loop-thumbnail-slider').find('.carousel').on('slid.bs.carousel',function(){
					var $indicators = $(this).closest('.product-container-inner').find('.carousel-indicators');
					$indicators.find('.active').removeClass('active');
					var $items = $(this).find('.carousel-inner').children('.item');
					var activeIndex = $items.index($(this).find('.item.active'));
				    var $nextIndicator = $($indicators.children()[activeIndex]);
			...
			if(parseInt(dhL10n.has_site_icon) > 0){
				var favicon = new Favico({animation:'slide'});
				$(document.body).bind( 'added_to_cart',function(event, fragments, cart_hash){
					try {
						if(fragments){
							if(fragments.cart_contents_count > 0){
								favicon.badge(fragments.cart_contents_count);
							}else{
								favicon.reset();
							}
						}else{
				...
				if ( parseInt(dhL10n.current_count) > 0 ){
					favicon.badge(dhL10n.current_count);
				}
				
			}
			var self = this;
			$('figure .shop-loop-quickview ,figure .add_to_wishlist,figure .yith-wcwl-wishlistexistsbrowse a,figure .yith-wcwl-wishlistaddedbrowse a').each(function(){
				var $this = $(this);
			...
			$('.shop-toolbar .view-mode a').on('click',function(e){
				...
				if($(this).hasClass('list-mode')){
					...
					$('.shop-loop').removeClass('grid');
				}else{
					$('.shop-loop').addClass('grid');
			...
			$('body').bind('added_to_cart',function(){
				...
				$('.minicart').fadeIn(500);
				$('.minicart-side').addClass('open');
				//window.clearTimeout(dh_added_to_cart_timeout);
				dh_added_to_cart_timeout = setTimeout(function(){
						$('.minicart-side').removeClass('open');
						$('.minicart').stop(true,true).fadeOut(500);
				},5000);
				
			});
			$(document).on("mouseenter", ".minicart-side", function() {
				clearTimeout(dh_added_to_cart_timeout);
			});
			$(document).on("mouseenter", ".minicart", function() {
				clearTimeout(dh_added_to_cart_timeout);
			});
			...
			$(document).on('click','.minicart .remove',function(e){
				  ...
				  $.ajax({
						...
						success: function(response){
							...
							$(".minicart-side .minicart").html(response.minicart);
