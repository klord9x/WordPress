...
function dh_do_not_reply_address(){
	return apply_filters( 'dh_do_not_reply_address', 'noreply@example.com' );
}
...
function dh_get_search_form($ajax=true){
	$search_form = '<form method="GET" class="searchform search-ajax" action="'.esc_url( home_url( '/' ) ).'" role="form">
					...
					<input type="submit" class="searchsubmit hidden" name="submit" value="'.esc_attr__( 'Search', 'sitesao' ).'" />
	...
	<div id="respond-wrap">
		<?php 
			...
			$comments_args = array(
					...
					'logged_in_as'		   => '<p class="logged-in-as">' . sprintf( esc_attr__( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'sitesao' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
					'title_reply'          => sprintf('<span>%s</span>',esc_attr__( 'Leave a reply', 'sitesao' )),
					'title_reply_to'       => sprintf('<span>%s</span>',esc_attr__( 'Leave a reply to %s', 'sitesao' )),
					'cancel_reply_link'    => esc_attr__( 'Click here to cancel the reply', 'sitesao' ),
					'label_submit'         => esc_attr__( 'Submit', 'sitesao' ),
					'comment_field'		   => '<p class="comment-form-comment"><textarea placeholder="'.esc_attr__('Comment','sitesao').'"  class="form-control" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
					'must_log_in'		   => '<p class="must-log-in">' .  sprintf( esc_html__( 'You must be %1$slogged in%2$s to post a comment.', 'sitesao' ), '<a href="'.wp_login_url( apply_filters( 'the_permalink', get_permalink( ) ) ).'>','</a>' ) . '</p>',
			);
...
function dh_post_nav() {
	...
	<nav class="post-navigation" role="navigation">
		<div class="row">
			<?php $prev_link = get_previous_post_link( '%link', _x( 'Previous post', 'Previous post link', 'sitesao' ) ); ?>
			<div class="col-sm-6">
			<?php if($prev_link):?>
				<div class="prev-post">
			...
			<?php $next_link = get_next_post_link( '%link', _x( 'Next post', 'Next post link', 'sitesao' ) ); ?>
			<div class="col-sm-6">
