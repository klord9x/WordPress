					...
					<?php if ( have_posts() ) : ?>
						...
						<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> itemtype="<?php echo dh_get_protocol() ?>://schema.org/Article" itemscope="">
							<div class="hentry-wrap">
								<?php 
								if(dh_get_theme_option('blog-show-feature',0)){
									dh_post_featured();
								}
								?>
								...
								<div class="entry-content" data-itemprop="mainContentOfPage">
									<?php the_content( sprintf(esc_html__( 'Continue reading %1$s&rarr;%2$s', 'loja' ),'<span class="meta-nav">','</span>' )) ?>
									<?php wp_link_pages( array( 'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'loja' ), 'after' => '</div>' ) ); ?>
								</div>
						 ...
						 ?>
					<?php endif;?>
					<?php if(dh_get_theme_option('show-postnav',0)) dh_post_nav();?>
