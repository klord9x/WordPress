...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-dh_disable_variation_gallery="<?php echo esc_attr(get_post_meta(get_the_ID(),'_dh_disable_variation_gallery',true)) ?>" data-product_id="<?php echo absint( $product->id ); ?>" data-product_images="<?php echo esc_attr(DH_Woocommerce_Product_Variable::instance()->get_product_image($product)) ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<p class="stock out-of-stock"><?php esc_html_e( 'This product is currently out of stock and unavailable.', 'loja' ); ?></p>
	<?php else : ?>
					...
					<tr>
						...
						<td class="value">
							...
							<?php
								...
								wc_dropdown_variation_attribute_options( array('show_option_none'=>sprintf(esc_html__('&ndash;&ndash; Select %s &ndash;&ndash;','loja'),wc_attribute_label( $attribute_name )), 'options' => $options, 'attribute' => $attribute_name, 'product' => $product, 'selected' => $selected ) );
							?>
							<i class="fa fa-angle-down"></i>
							</div>
							<?php 
							echo end( $attribute_keys ) === $attribute_name ? '<a class="reset_variations" href="#">' . esc_html__( 'Clear selection', 'loja' ) . '</a>' : '';
							?>
						</td>
					</tr>
		        <?php endforeach;?>
