...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		echo dh_print_string($prepend) . dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Products tagged &ldquo;', 'loja' ) . $queried_object->name . '&rdquo;' .'</span>'. dh_print_string($after);
		...
		if ( is_search() ) {
			...
			//echo dh_print_string($before) . '<span typeof="v:Breadcrumb"><a href="' . get_post_type_archive_link( 'product' ) . '" property="v:title" rel="v:url"><span>' . $_name . '</span></a></span>' . $delimiter . __( 'Search results for &ldquo;', 'loja' ) . get_search_query() . '&rdquo;' . dh_print_string($after);
			...
			echo dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Search results for &ldquo;','loja' ) . get_search_query() . '&rdquo;'  .'</span>'. dh_print_string($after);
		...
		echo dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Error 404', 'loja' ) .'</span>'. dh_print_string($after);
		...
		echo dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Search results for &ldquo;', 'loja' ) . get_search_query() . '&rdquo;'.'</span>'. dh_print_string($after);
			...
			echo dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Posts tagged &ldquo;', 'loja' ) . single_tag_title('', false) . '&rdquo;' .'</span>'. dh_print_string($after);

	} elseif ( is_author() ) {

		$userdata = get_userdata($author);
		echo dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Author:', 'loja' ) . ' ' . $userdata->display_name .'</span>'. dh_print_string($after);

	}

// 	if ( get_query_var( 'paged' ) )
// 		echo '<span typeof="v:Breadcrumb" property="v:title">'. ' (' . esc_html__( 'Page', 'loja' ) . ' ' . get_query_var( 'paged' ) . ')'.'</span>';
