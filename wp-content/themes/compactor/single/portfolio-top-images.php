        ...
        <div class="large-12 small-12 columns">
            <?php if (have_posts()) : ?>
                <?php while (have_posts()) : the_post(); ?>
                    <article class="single-portfolio top-grid-portfolio">

                        <div class="row">
                            <ul class="wd-post__thumbnail--gallery large-up-3">
                                <?php
                                ...
                                if (isset($portfolio_image_gallery_array) && count($portfolio_image_gallery_array) != 0):
                                    foreach ($portfolio_image_gallery_array as $gimg_id):
                                        echo '<li class="wd-gallery-image-holder column column-block"><a class="portfolioex" href="' . wp_get_attachment_image_url($gimg_id, 'full') . '" data-lightbox="roadtrip" >' . wp_get_attachment_image($gimg_id, 'compactor_single-gt-portfolio', false) . '</a></li>';
                                    endforeach;
                                endif;
                                ?>
                            </ul>
