...
if (!function_exists('compactor_add_mega_menu_support')):

    function compactor_add_mega_menu_support()
    {


        add_filter('wp_setup_nav_menu_item', 'compactor_add_custom_nav_fields');

        add_action('wp_update_nav_menu_item', 'compactor_update_custom_nav_fields', 10, 3);

        add_filter('wp_edit_nav_menu_walker', 'compactor_edit_menu_walker', 10, 2);
...
add_action('init', 'compactor_add_mega_menu_support');


/* Add custom fields to menu */

if (!function_exists('compactor_add_custom_nav_fields')):

    function compactor_add_custom_nav_fields($menu_item)
    {


        $menu_item->mega_menu_icon = get_post_meta($menu_item->ID, '_compactor_mega_menu_icon', true) ? get_post_meta($menu_item->ID, '_compactor_mega_menu_icon', true) : '';

        $menu_item->mega_menu_bg_image = get_post_meta($menu_item->ID, '_compactor_mega_menu_bg_image', true) ? get_post_meta($menu_item->ID, '_compactor_mega_menu_bg_image', true) : '';

        $menu_item->mega_menu = get_post_meta($menu_item->ID, '_compactor_mega_menu', true) ? 1 : 0;
...
if (!function_exists('compactor_update_custom_nav_fields')):

    function compactor_update_custom_nav_fields($menu_id, $menu_item_db_id, $args)
    {


        $value = isset($_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id]) ? $_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id] : '';
        update_post_meta($menu_item_db_id, '_compactor_mega_menu_icon', $value);


        $value = isset($_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id]) ? $_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id] : '';
        update_post_meta($menu_item_db_id, '_compactor_mega_menu_bg_image', $value);


        $value = isset($_REQUEST['menu-item-mega-menu'][$menu_item_db_id]) ? 1 : 0;
        update_post_meta($menu_item_db_id, '_compactor_mega_menu', $value);
...
if (!function_exists('compactor_edit_menu_walker')):

    function compactor_edit_menu_walker($walker, $menu_id)
    {


        class compactor_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit
        {
            ...
            public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
            {
                ...
                $mega_menu_html .= '</select>
		                ...
		                ' . esc_html__('display icone', 'compactor') . '</label>
                ...
                $mega_menu_html .= '<p class="field-custom description description-wide">
			                ...
			                ' . esc_html__('Mega Menu (Background image)', 'compactor') . '
                ...
                $mega_menu_html .= '<p class="field-custom description description-wide">
		                ...
		                ' . esc_html__('Mega Menu (classic)', 'compactor') . '
        ...
        return 'compactor_Walker_Nav_Menu_Edit';
