<?php
function compactor_google_fonts_array()
{
    $google_fonts = array(
        array(
            "name" => "ABeeZee",
...
function wd_Font_family()
{
    $font = array();
    foreach (compactor_google_fonts_array() as $fonts) {
        $font[$fonts["name"]] = $fonts["name"];
    }
    return $font;
}

function wd_Font_subsets($font)
{
    $subsets = array();
    foreach (compactor_google_fonts_array() as $fonts) {
        if ($fonts["name"] == $font) {
...
function wd_Font_Style($font)
{
    $style = array();
    foreach (compactor_google_fonts_array() as $fonts) {
        if ($fonts["name"] == $font) {
            foreach ($fonts["variants"] as $variants) {
                $style[$variants["style"]] = $variants["style"];
            }
        }
    }
    return $style;
...
function wd_font_weight($font)
{
    $weights = array();
    foreach (compactor_google_fonts_array() as $fonts) {
        if ($fonts["name"] == $font) {
            foreach ($fonts["variants"] as $variants) {
                foreach ($variants["weight"] as $weight) {
...
function wd_font_weight_name($weight)
{
    switch ($weight) {
        case "100":
            return "Thin 100";
