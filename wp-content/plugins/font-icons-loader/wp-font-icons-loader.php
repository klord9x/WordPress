...
Plugin URI: http://pix-theme.com/wordpress/proagency
...
function pixfil_fonts_activate(){
    ...
    update_option('fil_font_icons', $active_fonts);
}
        ...
        if( file_exists($fil_dir . $val) && $val != '' ){
            ...
            switch( $key ) {
                    ...
                    add_filter( 'vc_iconpicker-type-pixfontawesome', function($icons) use ($file_style){
                        ...
                        if (isset($matches) && !empty($matches)){
                            if(isset($matches[0]) && isset($matches[1])){
                                $classes = $matches[0]; $values = $matches[1];
	            ...
	            case 'stroke':
                    ...
                    break;
                default:
                    add_filter( 'vc_iconpicker-type-pixcustom', function($icons) use ($file_style){
                        $typicons_icons = array();
                        ...
                        foreach($dirlist as $val){
                            if( is_dir($fil_dir . $val) && !in_array($val, array('.', '..')) ){
                                ...
                                if(!empty($files_html)) {
                                    ...
                                    $html_path = explode( $fil_dir, $info['dirname'].'/'.$info['basename']);
	                                $html = $html_path[1];
                                }
